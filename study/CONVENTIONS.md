...
> ✍️ **글 쓰는 법(기억에 남고 흐름 좋게)은 [writing-guide.md](writing-guide.md) 참고** — SUCCESs·7단계 템플릿·체크리스트.
...
## 1.7 링크·사이드바 전략 (사이드바 폭발 방지) ⭐

깊이 파는 페이지가 늘어도 왼쪽 메뉴가 무한정 커지지 않게:
- **사이드바 = 큐레이션된 "학습 척추"만** — 개념(concept)·Q&A·대표 딥다이브. 모든 페이지를 넣지 않는다.
- **세부 파생 심화 페이지는 사이드바에 넣지 않고** 본문 **인라인 링크(`🔎 더 깊이 →`) + 검색**으로만 도달 (Obsidian식). 예: `ai-ml/deep-representation.md`.
- 딥다이브가 여러 개면 **접기(collapsed) 하위 그룹**으로 묶는다. 예: `🔬 심화 (딥다이브)` 그룹 하나로 접힘.
- **아주 작은 심화**는 새 페이지 대신 같은 문서 안 **`<details>` 접기 블록**으로:
  ```
  <details><summary>더 깊이: ○○</summary>

  내용...

  </details>
  ```
- 규칙: 개념 문서에서 더 파고들 지점마다 **인라인 링크 한 줄**을 남긴다(양방향이면 심화 문서에서도 상위 개념으로 링크).
