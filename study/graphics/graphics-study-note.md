...
> **연결**: AI 학습 노트(`../ai-ml/ml-attention-study-note.md`)와 GPU를 공유한다 — 12단원 참고.
...
**렌더링(rendering)**은 3D로 정의된 장면(점·면·재질·빛)을 2D 화면의 픽셀 색으로 바꾸는 계산이다. 방식은 크게 둘. **래스터화(rasterization)**는 "도형을 화면에 빠르게 칠하는" 방식으로 게임 등 실시간에 쓰이고, **레이트레이싱(ray tracing)**은 "빛의 경로를 따라가 사실적으로 계산하는" 방식으로 영화·고품질에 쓰인다. 둘 다 대량의 병렬 계산이라 **GPU**가 담당하며, 바로 이 GPU가 오늘날 AI 학습에도 함께 쓰인다.
