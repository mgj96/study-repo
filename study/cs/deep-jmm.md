...
> 🔀 **git 비유 — CAS는 `git push`다.** 내가 커밋을 push할 때 **원격이 안 움직였으면 성공(커밋)**, 누가 먼저 push해서 원격이 바뀌었으면 **거부** → `pull` 받아 **다시 push(재시도)**. "기대한 상태 그대로면 커밋, 아니면 재시도"가 정확히 compare-and-swap이다(= **낙관적 락**). 반대로 `synchronized`는 "이 파일 작업하는 동안 남들 손 못 대게 잠그기"(**비관적 락**).
