...
신경망은 처음엔 **아무렇게나** 답해요. 그리고 "정답과 얼마나 틀렸나(오차)"를 재고, **그 오차를 뒤에서 앞으로 거꾸로 되짚으며** "너는 이만큼 잘못했으니 이 방향으로 살짝 고쳐"라고 각 연결에 알려줘요. 이걸 수천 번 반복하면 점점 똑똑해져요. 이 "거꾸로 되짚기"가 **역전파(backpropagation)**, "살짝 고치기"가 **경사하강(gradient descent)**이에요. 아래는 그 수학이에요.
