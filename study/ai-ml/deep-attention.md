...
문장을 이해하려면 **어떤 단어가 어떤 단어와 관련 있는지**를 알아야 해요. "그것이 피곤했다"에서 "그것"이 뭘 가리키는지 같은 거요. **어텐션**은 각 단어가 문장의 모든 단어를 둘러보고 "누구에게 집중할까?"를 **점수**로 매기는 방법이에요. 손전등을 관련 단어에 세게 비추는 거죠. 이 손전등을 **여러 개(멀티헤드)** 켜고 **여러 층** 쌓은 게 **트랜스포머**고, ChatGPT·Claude가 다 이걸로 만들어졌어요.

아래부터는 "정확히 어떻게 계산하나"를 수식까지 파고듭니다.
...
```
입력 토큰 → 임베딩 + 위치인코딩
  → [인코더 층 ×6]  (셀프어텐션 → FFN, 각 잔차+LayerNorm)
  → 인코더 출력(문맥 표현)
디코더: 출력 토큰 → 임베딩+위치
  → [디코더 층 ×6] (마스크드 셀프어텐션 → 인코더-디코더 어텐션 → FFN)
  → 선형 + softmax → 다음 토큰 확률
```
