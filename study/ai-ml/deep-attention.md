...
*(도식 설명: 단어들이 서로를 훑어 관련도 점수를 매기고 → 관련 깊은 단어에 집중해 정보를 섞어 → 문맥이 담긴 새 표현을 만든다. 이 과정을 손전등 여러 개·여러 층으로 쌓으면 트랜스포머다.)*
...
*(도식 설명: 단어 임베딩에서 Q·K·V 세 벡터를 만들고 → Q와 모든 K를 내적해 관련도 점수 → √dk로 나눈 뒤 softmax로 집중 비율(합=1) → 그 비율로 각 V를 가중합해 문맥이 섞인 새 표현을 얻는다.)*
...
*(도식 설명: 입력 토큰이 임베딩+위치인코딩을 거쳐 인코더 6층(셀프어텐션+FFN)을 지나 문맥 표현이 되고, 디코더 6층(마스크드 어텐션 → 인코더-디코더 어텐션 → FFN)이 그 문맥을 참고해 다음 토큰 확률을 만든다.)*
